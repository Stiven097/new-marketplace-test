Trestle.resource(:transactions) do
  ...
  menu do
    item :transactions, icon: "fa fa-money", label: "Widthdraw"
  end
  ...
  controller do
    def approve
      ...
      response = EXPRESS_GATEWAY.transfer(transfer_amount * 100, transaction.buyer.paypal, 
                                          subject: 'Payment from Marketplace', 
                                          note: "This payment (#{transfer_amount}) comes from the last job you completed.")
      ...
      if response.success?
        ...
        flash[:message] = "The transaction has been approved."
      end
    ...
    def reject
      ...
      flash[:message] = "The transaction has been rejected."
      redirect_to transactions_admin_index_path
    end
  end

class OffersController < ApplicationController
  ...
  def create
    ...
    if Offer.exists?(user_id: current_user.id, request_id: offer_params[:request_id])
      redirect_to request.referrer, alert: "You can only make one request at the moment"
    end

    @offer = current_user.offers.build(offer_params)
    if @offer.save
      redirect_to my_offers_path, notice: "Your offer has been successfuly submitted"
    else
  ...
  def accept
    if @offer.pending?
      ...
      if charge(@offer.request, @offer)
        ...
        flash[:alert] = "Cannot process your offer"
      end
    end
  ...
  def reject
    if @offer.pending?
      @offer.rejected!
      flash[:notice] = "The offer has been rejected"
    end
    redirect_to request.referrer
  end
